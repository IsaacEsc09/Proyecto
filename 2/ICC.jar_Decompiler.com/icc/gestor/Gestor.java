...
public class Gestor {
   ...
   public Gestor() throws IllegalSizeException {
      String s = "";
      s = s + this.loadLibros();
      s = s + this.loadDiscos();
   ...
   public String loadLibros() {
      ...
      this.LIBROS = new Libro[32];
      this.index_l = 0;

      for(int i = 0; i < l.length; ++i) {
         String[] array = l[i].split(",");
         Libro art = new Libro(array[0], array[1], array[2], array[3]);
         if (i > 32) {
   ...
   public String loadDiscos() {
      ...
      this.DISCOS = new Disco[32];
      this.index_d = 0;

      for(int i = 0; i < l.length; ++i) {
         String[] array = l[i].split(",");
         Disco art = new Disco(array[0], array[1], array[2], Integer.parseInt(array[3]));
         if (i > 32) {
   ...
   public String loadPeliculas() {
      ...
      this.PELICULAS = new Pelicula[32];
      this.index_p = 0;

      for(int i = 0; i < l.length; ++i) {
         String[] array = l[i].split(",");
         Pelicula art = new Pelicula(array[0], array[1], array[2], Integer.parseInt(array[3]));
         if (i > 32) {
   ...
   public void agregarLibro(Libro l) throws IllegalSizeException {
      if (this.index_l >= 32) {
   ...
   public void agregarDisco(Disco d) throws IllegalSizeException {
      if (this.index_d >= 32) {
         throw new IllegalSizeException("Capacidad maxima alcanzada.");
      } else {
   ...
   public void agregarPelicula(Pelicula p) throws IllegalSizeException {
      if (this.index_p >= 32) {
         throw new IllegalSizeException("Capacidad maxima alcanzada.");
      } else {
